-- TỦ QUÀ — BẢN GỘP DÁN 1 LẦN trong Supabase SQL Editor (role postgres mặc định).
-- = sql_tuqua_chuyen_chu.sql + migration 202608300908_tu_qua_v1.sql nối lại, để Thùy chỉ dán 1 phát
-- (kể cả từ điện thoại) là DB xong. Đã verify trọn bộ trên replica PG16 (PLAN-tuqua.md §4).
--
-- SAU KHI DÁN, lúc nào ngồi máy thì chạy để ghi sổ + cập nhật schema (KHÔNG gấp):
--   npm run migrate   ← migration idempotent, chạy lại vô hại, mục đích chỉ là ghi sổ _migrations
--   npm run schema    ← refresh schema.md, commit
-- Còn lại: cấp leaf `tu_qua` cho role OPS ở màn Phân quyền (app ẩn tab nếu chưa cấp).
...
-- ═══════════════════════════════════════════════════════════════════════════
-- PHẦN 2 — nội dung migration 202608300908_tu_qua_v1.sql (nguyên văn)
-- Chạy dưới postgres thay vì claude_build: guard so current_user đã đổi owner xong nên...
-- ⚠ KHÔNG nguyên văn được chỗ guard — xem dòng set role ngay dưới.
set role claude_build;   -- chạy phần migration DƯỚI claude_build (postgres là member nên set được)
                         -- → object mới (bảng log, fn, policy) thuộc claude_build y như npm run migrate
...
reset role;
-- Kiểm nhanh sau khi chạy (phải thấy hàm fn_tuqua_doi + bảng qlht_log):
-- select proname from pg_proc where proname like 'fn_tuqua%' order by 1;
select 'TU QUA — DB SETUP XONG' as ket_qua;
