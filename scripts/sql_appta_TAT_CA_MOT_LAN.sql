-- ============================================================================
-- APP TA + PH NỘP BTVN — BẢN DÁN 1 LẦN (Supabase Dashboard → SQL Editor → Run).
-- Gộp: ① role ph_nop  ② toàn bộ migration 202608302120 (chạy dưới `set role
-- claude_build` để object thuộc claude_build y như npm run migrate — migration
-- idempotent nên sau này `npm run migrate` chạy lại vô hại, chỉ ghi sổ _migrations)
-- ③ bucket private btvn-nop + policies  ④ grant cho ph_nop.
-- ⚠ TRƯỚC KHI RUN: sửa 'DOI_MAT_KHAU_NAY' thành mật khẩu mạnh TỰ ĐẶT (dòng ngay dưới).
--   Mật khẩu này đưa vào env SERVER của bkdemy-ph — KHÔNG commit, KHÔNG xuống client.
-- ============================================================================

-- ① ROLE ph_nop — đường ghi hẹp cho server bkdemy-ph (chỉ EXECUTE 1 hàm nộp, không SELECT gì)
do $$ begin
...
-- postgres cần là member của claude_build để set role (đã grant ở vụ Tủ quà — chạy lại vô hại)
grant claude_build to postgres;

-- ② MIGRATION 202608302120 — nguyên văn, chạy dưới claude_build
set role claude_build;
...
-- ③ BUCKET btvn-nop — PRIVATE (ảnh bài làm HS). Staff đọc/ghi qua authenticated;
--    PH xem qua signed URL (server bkdemy-ph ký). KHÔNG policy delete — ảnh nộp là bằng chứng.
...
-- ④ GRANT cho ph_nop (hàm đã tồn tại chắc chắn vì ② vừa chạy xong)
grant execute on function public.fn_btvn_nop_tao(uuid, uuid, text[]) to ph_nop;

-- Xong. Kiểm nhanh: 2 dòng dưới phải trả về 1 hàng mỗi dòng.
select 'role ph_nop' as kiem, count(*) from pg_roles where rolname = 'ph_nop';
select 'bucket btvn-nop' as kiem, count(*) from storage.buckets where id = 'btvn-nop';
