...
const tinh = (soHS, model) => {
  const g = GIA[model]
  const vaoUoc = 1400 * soHS + 1600            // đo thật 11B1: 6.245 vào cho 4 em
...
ok(tinh(15, 'claude-sonnet-5').maxTokens > 5025 * 15 / 4 * 0.9, 'lớp 15 em có đủ trần cho ~19.000 token ra (ca vừa hỏng)')
const xau = tinh(40, 'claude-opus-4-8')
ok(xau.chan && xau.quaDong, 'ca 40 em BỊ CHẶN vì vượt trần token (gọi cũng chắc chắn bị cắt)')
ok(tinh(25, 'claude-opus-4-8').chan && !tinh(25, 'claude-opus-4-8').quaDong, '25 em × Opus bị chặn vì QUÁ TRẦN TIỀN, không phải vì quá đông')
