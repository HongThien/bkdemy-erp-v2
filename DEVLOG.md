...
**FIX LANE v2 — Claude tự fix bug + làm tính năng theo order (Thùy chốt 29/08, worktree `wt-fix-lane`, branch `feat/fix-lane-v2`):**
- Bối cảnh: hạ tầng auto-report Pha 1+2 (bao_loi + BaoLoiScreen + FIX_LANE + reports_pull/report_set) TRÊN MAIN từ 06, nhưng chưa chạy thật lần nào — 34 report `moi` dồn 21/7→26/8, 0 `cho_fix`. Thùy chốt 4 quyết định qua hỏi-đáp: **① on-demand trước** (bảo "chạy fix-lane" là chạy; cron/cloud sau khi nghiệm) · **② test dev READ-ONLY** (dev chung DB production; ca bắt buộc ghi mới verify được → agent DỪNG hỏi Thùy từng ca) · **③ migration SOẠN KHÔNG ÁP** (file nằm trong branch, Thùy áp lúc duyệt — DB bất khả xâm phạm với lane) · **④ duyệt qua PR + Vercel preview** (cần `gh auth login` 1 lần — hiện CHƯA login, lane chưa mở PR được).
- Build: mig `202608292149_bao_loi_loai_va_order` — `bao_loi.loai` ('bug'|'yeu_cau', default 'bug', CHECK kèm — đúng bài "union type TS phải có CHECK đi kèm") · `baoloi.ts` thêm `LoaiBaoLoi` + `createOrderTinhNang` (order Thùy vào THẲNG `cho_fix` — chính người duyệt tạo, khỏi qua cổng 2) · BaoLoiScreen: nút "➕ Order tính năng" (form mô tả + leaf tuỳ chọn) + badge tím "Yêu cầu" · `reports_pull.mjs` in kèm loại · FIX_LANE.md viết lại toàn bộ theo 4 quyết định (thêm bước TEST TRÊN DEV bắt buộc + bằng chứng test vào fix_note/PR, guardrail DB, checklist setup).
- ✅ tsc + vite build pass. **CHƯA làm được trong phiên (permission classifier chặn):** áp mig (`npm run migrate` — 1 file treo duy nhất, additive) + `npm run schema` + verify UI trên dev server. Thùy/phiên sau chạy: `npm run migrate && npm run schema` trong wt-fix-lane rồi test nút Order. **Chú ý:** chưa áp mig thì form Order sẽ 400 (cột loai chưa có); list/badge vẫn chạy (loai undefined → ẩn badge).
