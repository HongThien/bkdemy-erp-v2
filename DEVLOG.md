...
- **Dọn buổi rác 9S1 + nhãn MT khi tìm buổi (07-28):** 9S1 ngày 08/07 có 2 buổi — 1 buổi `loai='mt'` (mô hình MT-riêng CŨ, trước 07-08) sót lại + 1 buổi thường (MT đã vào phase='mt', 18 câu). Toàn hệ chỉ 1 buổi `loai='mt'` này (ca lẻ), không Elo/EXP/ky_thi → **huỷ** (trang_thai='huy', reversible). **Nhãn MT:** `timBuoiTheoLop` thêm cờ `coMT` (buổi có câu phase='mt', 1 query), `BuoiTimRow` hiện badge **🏆 MT** → tìm buổi thấy ngay buổi nào có sát hạch.
