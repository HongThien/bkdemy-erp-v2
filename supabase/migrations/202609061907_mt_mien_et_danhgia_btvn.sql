-- ============================================================================
-- 202609061907 — Buổi có gán MT: MT THAY HẲN ingame/ET/Đánh giá/BTVN, KHÔNG cộng thêm.
-- ----------------------------------------------------------------------------
-- CEO chốt 09/06: "buổi có MT thì không có ET, đánh giá và nhận xét — nói chung là
-- không có hoạt động nào khác" → CHỈ còn 1 việc "Chấm MT" cho buổi đó, GV không còn
-- việc gì (ingame/đánh giá vốn là việc GV), TA chỉ còn Chấm MT (thay cho ingame/ET/BTVN).
-- TRƯỚC ĐÂY (0091 + chuỗi db_dong_phase*): MT là CỘNG THÊM — buổi có MT vẫn đòi đủ
-- ingame/et/danhgia/btvn NHƯ BUỔI THƯỜNG, cộng thêm 1 task MT riêng. Sai với ý CEO,
-- và đúng như TA phản ánh: buổi MT vẫn còn deadline ET/đánh giá trên app.
--
-- "Có gán MT" = tai_lieu(loai='mt_buoi') khớp (lop_id, ngay) của buổi — ĐÚNG nguồn
-- gami.ts đang dùng để hiện task "Chấm MT" (KHÔNG dùng gami_session_problems.phase='mt'
-- như code cũ: cột đó chỉ có DỮ LIỆU sau khi TA đã mở buổi và đồng bộ lưới câu — nếu
-- lấy làm điều kiện "miễn" thì trước lúc TA đồng bộ xong, buổi vẫn bị đòi đủ 4 mốc kia,
-- ngay khi đồng bộ xong thì 4 mốc đó biến mất đột ngột — cùng 1 buổi nhưng chuẩn hoàn
-- tất đổi giữa chừng. tai_lieu là mốc CỐ ĐỊNH ngay từ lúc gán MT, không đổi lại.
--
-- ĐỤNG 4 CHỖ:
--   ① fn_buoi_recompute_hoan_tat — hoàn tất chỉ cần mt_dong_at khi có gán MT.
--   ② fn_dong_phase — khoá tuần tự ET bỏ qua buổi có gán MT (buổi đó vĩnh viễn không
--      có tab ET để đóng — không bỏ qua thì mọi buổi SAU nó bị kẹt "chưa đóng ET" mãi).
--   ③ fn_ta_dashboard — _ta_viec: buổi có MT chỉ sinh 1 việc tab='mt' (thay vì 3 việc
--      ingame/et/btvn).
--   ④ fn_gv_dashboard — _gv_viec: buổi có MT bị loại khỏi mẫu số hẳn (GV không có việc).
-- gami.ts (client, cùng đợt sửa): getMyTasks/listAllStaffTasks bỏ hẳn nhánh
-- ingame/et/btvn/danhgia khi buổi có gán MT, chỉ còn "Chấm MT" cho vai tg.
--
-- MẤT GÌ (Luật xoá): không — create or replace 4 function, không đổi schema/dữ liệu.
-- ============================================================================

-- ① Hoàn tất: buổi có gán MT chỉ cần mt_dong_at.
...
begin
  select trang_thai, lop_id, ngay, ingame_dong_at, et_dong_at, danh_gia_xong_at, btvn_dong_at, mt_dong_at
    into b from buoi_hoc where id = p_buoi_id;
  if b is null or b.trang_thai = 'huy' then return; end if;
  select exists(select 1 from tai_lieu tl where tl.loai = 'mt_buoi' and tl.lop_id = b.lop_id and tl.ngay = b.ngay)
    into v_has_mt;
  v_next := case
    when v_has_mt then (case when b.mt_dong_at is not null then 'hoan_tat' else 'mo' end)
    when b.ingame_dong_at is not null and b.et_dong_at is not null
          and b.danh_gia_xong_at is not null and b.btvn_dong_at is not null
     then 'hoan_tat' else 'mo'
  end;
...
-- ② fn_dong_phase v6: khoá tuần tự ET bỏ qua buổi có gán MT (nền = v5, 202608312039).
...
begin
  ...
  if p_phase = 'et' and b.loai = 'thuong' then
    perform 1 from buoi_hoc bh where bh.lop_id = b.lop_id and bh.loai = 'thuong' and bh.trang_thai <> 'huy'
      and bh.ngay < b.ngay and bh.et_dong_at is null
      and not exists (select 1 from tai_lieu tl where tl.loai = 'mt_buoi' and tl.lop_id = bh.lop_id and tl.ngay = bh.ngay)
      limit 1;
  ...
  if p_phase in ('et', 'mt') and b.loai in ('thuong', 'mt') and v_n > 0 and v_probs > 0 and not v_online_et then
...
-- ③ fn_ta_dashboard: buổi có gán MT → 1 việc (tab='mt') thay vì 3 việc (ingame/et/btvn).
...
begin
  ...
  with mt_buoi as (
    select distinct b.id as buoi_id
    from buoi_hoc b
    join tai_lieu tl on tl.loai = 'mt_buoi' and tl.lop_id = b.lop_id and tl.ngay = b.ngay
  ),
  viec as (
    select distinct pc.nhan_su_id, b.id as buoi_id, l.ten_lop, b.ngay, t.tab,
      case t.tab when 'ingame' then b.ingame_dong_at when 'et' then b.et_dong_at
                  when 'mt' then b.mt_dong_at else b.btvn_dong_at end as dong_at,
      case t.tab
        ...
        when 'mt' then ((b.ngay)::text || ' 23:59')::timestamp at time zone 'Asia/Ho_Chi_Minh'
    ...
    cross join lateral (
      select 'mt' as tab where exists (select 1 from mt_buoi m where m.buoi_id = b.id)
      union all
      select t2.tab from (values ('ingame'), ('et'), ('btvn')) t2(tab)
      where not exists (select 1 from mt_buoi m where m.buoi_id = b.id)
    ) t
...
-- ④ fn_gv_dashboard: buổi có gán MT → loại khỏi mẫu số hẳn (GV không có việc ở buổi này).
...
begin
  ...
  with viec as (
    ...
    where b.loai = 'thuong' and b.trang_thai <> 'huy' and b.ngay >= v_tu and b.ngay < v_den
      and not exists (
        select 1 from tai_lieu tl where tl.loai = 'mt_buoi' and tl.lop_id = b.lop_id and tl.ngay = b.ngay)
  )
