-- ============================================================================
-- 202609151030_test_dau_vao_phieu_gv_tg_chinh — fn_test_dau_vao_phieu: lopDeXuat thêm `gvChinh` + `tgChinh`
--   ({hoTen, anhUrl} | null) = người `la_chinh` của lớp theo `phan_cong_lop` (vai_tro gv / tg), ảnh = `nhan_su.anh_url`
--   (avatar tài khoản nhân sự). CEO 15/09: phiếu gửi PH hiện 2 ảnh "Giáo viên" – "Giáo viên bổ trợ" dưới khối lớp.
--   Không có người chính ⇒ lấy người đầu theo tên; không có ai ⇒ null (phiếu bỏ ô đó). Sao chép nguyên hàm 202609131618.
