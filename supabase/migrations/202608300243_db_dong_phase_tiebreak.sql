-- ============================================================================
-- 202608300243 — vá fn_dong_phase: TIE-BREAK HẠNG TẤT ĐỊNH (hoc_sinh_id)
-- ----------------------------------------------------------------------------
-- VÌ SAO: bài test vàng (reopen+reclose buổi ET 28/08, rollback) cho thấy JS cũ xếp
--   hạng 2 HS HOÀ TUYỆT ĐỐI (cùng điểm thô + cùng Δ) theo thứ tự trả về NGẪU NHIÊN
--   của roster query (không order by) — tức hạng 4/5 của cặp hoà là hên xui mỗi lần
--   chạy, kéo EXP (bậc 4 = 250 vs bậc 5 = 225) đổi theo. Không thể "parity" với sự
--   ngẫu nhiên; chọn chuẩn TẤT ĐỊNH: hoà điểm + hoà Δ → hoc_sinh_id nhỏ xếp trên.
--   Áp cho cả history/reveal (đường Elo) lẫn phase xếp hạng không Elo.
-- MẤT GÌ (Luật xoá): không đụng dữ liệu — chỉ replace function.
-- ============================================================================
  ...
  if v_co_rank then
    if v_co_elo then
      ...
      select jsonb_agg(jsonb_build_object(
               ...
               'rank', row_number() over (order by r.points desc, r.hoc_sinh_id), 'exp', 0)
             order by r.points desc, r.hoc_sinh_id)
        into v_reveal
      from _dp_raw r;
    end if;
  else
