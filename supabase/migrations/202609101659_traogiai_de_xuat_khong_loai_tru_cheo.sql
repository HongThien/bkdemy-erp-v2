-- ============================================================================
-- 202609101659 — TRAO GIẢI: đề xuất từng giải ĐỘC LẬP, không loại trừ chéo giữa các đề xuất (CEO 10/09)
-- ----------------------------------------------------------------------------
-- VÌ SAO: CEO hỏi "7S3 sao không hiện HS tiến bộ?" — tháng 8 lớp 7S3 có 2 em lên hạng (Nguyễn Tường Anh +10,
--   Lê Thành An +3) nhưng cả 2 đã được ĐỀ XUẤT Xuất sắc, và bản 202609091431/1631 đề xuất THAM LAM theo ưu tiên
--   (1 em chỉ hiện ở 1 giải) ⇒ ô Tiến bộ trống dù có ứng viên. Sai ý CEO: người duyệt cần THẤY ứng viên để chọn
--   em đó nhận giải nào. Sửa: mỗi giải đề xuất độc lập, chỉ loại HS đã CHỐT (confirmed). Trùng đề xuất là bình
--   thường; tick chốt 1 giải xong, refetch nền tự rút em đó khỏi đề xuất giải kia (UNIQUE ở DB vẫn gác).
--   Đây là quay về đúng thiết kế mockup 22/08 (CEO duyệt), bỏ "tham lam" tao tự thêm 09/09.
--   Kèm: mỗi slot đã chốt trả thêm `congBoAt` (tab "Đã chốt giải" cần biết dòng đã công bố hay chờ chốt tháng).
-- MẤT GÌ (Luật xoá): không. create or replace fn_traogiai_thang (giữ nguyên slot tuỳ chỉnh 1631).
-- ============================================================================
...
begin
  ...
  -- Slot = đã chốt (theo duyet_at) + đề xuất từng giải ĐỘC LẬP (chỉ loại HS đã chốt; 1 em có thể được đề xuất ở 2 giải)
