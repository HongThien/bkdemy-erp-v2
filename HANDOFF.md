...
- **Phạm vi v1 (CEO chốt):** Toán, nhánh **Đại + Hình GT**. KHTN đối xứng sẵn nhưng chưa bật UI. **Hình học thuần (`hinh_hoc_bai`) ĐỨNG NGOÀI** — 3 cột compat `ma_chuyen_de/ten_chuyen_de/muc_do` (mig 202609181735) còn RỖNG nên không dựng được cây lọc.
...
- **CHƯA verify end-to-end:** chưa gọi `hs_sotay_cay` thật (claude_ro không có EXECUTE, không có tài khoản HS). Mọi kết luận về output là do nhân bản query. **Cần CEO mở app HS bằng 1 tài khoản khối 6-9.**
