...
- **Khối static (0015–0023):** `nhan_su`(+ma_ns NS001 tự sinh · anh_url) · `tai_khoan`(id=auth.uid→nhan_su) · `team`(seed 6) · **`vi_tri`**(team_id · ten=chức vụ · cap truong/pho/thanh_vien · **cha_id→vi_tri** · nhan_su_id NULL=trống) · `lop`(ten_lop·mon·khoi·bac FK lop_bac·co_so) · `phan_cong_lop`(ns×lop×vai_tro gv/tg·la_chinh) · `hoc_sinh`(ma_hs HS0001 tự sinh·khoi·dia_chi·truong_hoc·phu_huynh_id·anh_url) · `phu_huynh`(ma_ph PH0001) · `hoc_sinh_lop`(hs×lop·**muc_nang_luc_id**·trang_thai dang_hoc/da_roi) · `thoi_khoa_bieu`(lop·thu 2-8·giờ·hieu_luc_tu/den) · `muc_nang_luc`(12 mức seed). (`thanh_vien_team` đã DROP ở 0022.)
- **Migrations ĐÃ áp vào DB live (ĐỪNG chạy lại):** 0001–0019, 0021–0023. **0020 (bucket avatars) CHƯA chạy — Dashboard SQL Editor.** Bucket đã có: `kho-anh`, `kho-tailieu`.
