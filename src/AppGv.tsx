// AppGv — shell RIÊNG cho bundle app GIÁO VIÊN (PLAN-app-gv.md, Thùy chốt 31/08). Y hệt vai trò
// AppTa/AppOps: không import gì thuộc NhanSuHome/useStore/screens kho — bundle nhẹ. Quyền = CÙNG
// nguồn với ERP (my_quyen + phân công lớp vai 'gv'), không đẻ khái niệm quyền mới.
import { useEffect, useState } from 'react'
import type { Session } from '@supabase/supabase-js'
import { supabase } from './lib/supabase'
import Login from './auth/Login'
import GvHome from './screens/gv/GvHome'
...
export default function AppGv() {
  ...
  if (!session) return <Login staffOnly title="BK Giáo viên" subtitle="Đăng nhập nhân sự giáo viên" />
  if (gate === undefined) return <DangTai />
  if (gate === 'hs') return <ManThongBao text="Tài khoản này là học sinh — app này chỉ dành cho giáo viên." />
  if (gate === 'khong_link') return <ManThongBao text="Tài khoản chưa gắn với hồ sơ nhân sự nào — liên hệ quản trị." />
  return <GvHome profile={gate.profile} quyen={gate.quyen} />
}
