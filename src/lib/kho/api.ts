...
const FMT_RULES = [
  ...
  '- ⚠ CÔNG THỨC HOÁ HỌC (KHTN) có CHỈ SỐ DƯỚI (H₂O, CO₂, H₂SO₄, Fe₂O₃, Al₂(SO₄)₃…) PHẢI bọc trong $...$ và viết chỉ số dưới bằng dấu gạch dưới "_": "$H_2O$", "$CO_2$", "$H_2SO_4$", "$Fe_2O_3$", "$Al_2(SO_4)_3$". Chỉ số dưới NHIỀU KÝ TỰ phải bọc ngoặc nhọn: "$C_{12}H_{22}O_{11}$" (KHÔNG viết "C_12" — chỉ "1" bị hạ xuống, "2" vẫn cỡ thường). Ion/điện tích dùng "^": "$Fe^{2+}$", "$SO_4^{2-}$". TUYỆT ĐỐI KHÔNG để chỉ số hoá học trần ngoài $...$ (viết "H2O" thường) — chỉ số dưới sẽ mất, hiện thành số cỡ thường ngang hàng chữ cái.',
