...
export type Vai = 'checkin' | 'dangky' | 'quay' | 'quantro' | 'quaqua' | 'quanly' // 'quaqua' còn trong DB nhưng app KHÔNG dùng (Thùy 26/09: đổi quà không thuộc hệ này)
export type SuKien = {
...
export const daCheckin = (suKien: string) => rpc<{ nguoi_choi_id: string; ten: string; so: number; lop: string | null; checkin_at: string; xu_quay: number | null }[]>('fn_sk_da_checkin', { p_su_kien: suKien })
