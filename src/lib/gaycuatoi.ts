...
export type GayCuaToi = { ky: string; ledger: GayCuaToiEntry[]; deXuatCho: GayDeXuat[] }

// ky = 'YYYY-MM-01' (cùng quy ước gay.ts). Đề xuất chờ: lấy theo deadline_at rơi trong tháng.
export async function gayCuaToi(ky: string): Promise<GayCuaToi> {
  ...
  const [{ data: led, error: e1 }, { data: dx, error: e2 }, lois, hds] = await Promise.all([
    supabase.from('gay_ledger').select('*').eq('nhan_su_id', me).eq('ky', ky).order('created_at', { ascending: false }).limit(LIMIT),
    supabase.from('gay_de_xuat').select('*').eq('nhan_su_id', me).eq('trang_thai', 'cho')
      .gte('deadline_at', tu).lt('deadline_at', den).order('deadline_at', { ascending: false }).limit(LIMIT),
