// SHELL "CỦA TÔI" kiểu box (CEO 07/09 — đại cấu trúc app mobile, làm TA trước, dùng chung cho
// GV/OPS sau): header tháng ‹ › + chip ĐIỂM TÍCH LŨY + ô % đạt chuẩn thu nhỏ, dưới là lưới 6 box
// (Xếp hạng · Gậy · May mắn · Tiến trình · Shopping · Hướng dẫn). Bấm box → màn con, có nút ‹ quay
// lại. Box chưa mở (Đợt 2) hiện nhãn "sắp mở" — không giả vờ có dữ liệu.
// File này CHỈ là khung + các màn con dùng chung (Gậy, Hướng dẫn, Đạt chuẩn); Xếp hạng/Tiến trình
// do từng app truyền vào (props) vì dữ liệu khác nhau theo vai trò.
...
export function CuaToiHeader({ ym, ymNay, onYm, mau, pct, diemTichLuy, onPct, onBack }: {
  ym: string; ymNay: string; onYm: (ym: string) => void; mau: string
  pct: number | null | undefined; diemTichLuy: number | null; onPct: () => void; onBack: (() => void) | null
}) {
  ...
  return (
    <div className={`${mau} px-4 pb-3`} style={{ paddingTop: 'max(0.5rem, env(safe-area-inset-top))' }}>
      ...
      <div className="mx-auto mt-2 flex max-w-[1000px] gap-2">
        {/* Điểm tích lũy — Đợt 2 mới có công thức; trước đó hiện "—" chứ không bịa số */}
        <div className="flex flex-1 items-center gap-2 rounded-xl bg-white/15 px-3 py-2">
          ...
          <div className="leading-tight">
            ...
            <p className="text-[17px] font-extrabold text-white">{diemTichLuy == null ? '—' : diemTichLuy}</p>
          </div>
        </div>
...
// ── LƯỚI 6 BOX ────────────────────────────────────────────────────────────────
type BoxDef = { key: BoxKey; icon: string; label: string; sub?: string; bg: string; sapMo?: boolean; badge?: number }
...
// ── MÀN CON: ĐẠT CHUẨN (bar + 4 số + danh sách việc) — nội dung màn cũ dời vào đây ─────
export function DatChuanBox({ me, items, tabTen, lyDoTen, mauBar, chuThich }: {
  ...
  if (!me.tong) return <Trong>Tháng này chưa có việc nào được giao.</Trong>
  return (
    <div className="flex flex-col gap-3">
      <div className="rounded-2xl border border-slate-200/70 bg-white p-4 shadow-sm">
