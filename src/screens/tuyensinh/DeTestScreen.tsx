    ...
    <div className="mx-auto max-w-[900px] p-6">
      <div className="mb-4 flex flex-wrap items-center gap-2">
        <div>
          ...
          <p className="text-[12px] text-slate-400">Chọn 1 tài liệu có sẵn trong Kho làm đề (thường MT, không giới hạn loại). Đổi đề = chọn tài liệu khác, đề cũ ngừng dùng nhưng giữ lịch sử.</p>
        </div>
      ...
      {loading ? <p className="text-sm text-slate-400">Đang tải…</p> : list.length === 0 ? (
        ...
        <div className="grid gap-2.5">
          {list.map((d) => <DeCard key={d.id} d={d} onPrint={() => setPrintDoc({ id: d.taiLieuId, loai: d.taiLieuLoai })} onChanged={reload} />)}
        </div>
      )}

      {picking && <ChonTaiLieuModal khoi={khoi} mon={mon} onClose={() => setPicking(false)} onDone={async () => { setPicking(false); await reload() }} />}
...
function DeCard({ d, onPrint, onChanged }: { d: DeTest; onPrint: () => void; onChanged: () => void }) {
...
// ── Chọn 1 tài liệu CÓ SẴN trong Kho làm đề — mặc định lọc khối+môn đang xem, có thể mở rộng ──
function ChonTaiLieuModal({ khoi, mon, onClose, onDone }: { khoi: string; mon: string; onClose: () => void; onDone: () => void }) {
  const [all, setAll] = useState<TaiLieu[]>([])
  const [loading, setLoading] = useState(true)
  const [q, setQ] = useState('')
  const [taiKhoi, setTaiKhoi] = useState(true) // lọc đúng khối đang chọn — bỏ tick = xem mọi khối ("ngoại lệ cho chắc")
  const [picked, setPicked] = useState<TaiLieu | null>(null)
  const [ten, setTen] = useState('')
  const [busy, setBusy] = useState(false)
  const [err, setErr] = useState<string | null>(null)

  useEffect(() => { listAllTaiLieu(mon).then(setAll).finally(() => setLoading(false)) }, [mon])
  const filtered = all.filter((t) => (!taiKhoi || t.khoi === khoi) && (!q.trim() || t.ten.toLowerCase().includes(q.trim().toLowerCase())))

  function chon(t: TaiLieu) { setPicked(t); setTen(`Đề test đầu vào K${khoi} · ${mon} — ${t.ten}`) }
  async function xacNhan() {
    if (!picked || !ten.trim()) return
    setBusy(true); setErr(null)
    try { await datDeDangDung({ khoi, mon, ten: ten.trim(), taiLieuId: picked.id }); onDone() }
    ...
    return (
      <div className="fixed inset-0 z-[80] flex items-center justify-center bg-slate-900/40 p-4" onClick={onClose}>
        <div className="w-full max-w-[480px] rounded-2xl bg-white p-5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
          <div className="mb-3 text-[16px] font-semibold text-slate-800">Đặt làm đề đang dùng</div>
    ...
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-slate-900/40 p-4" onClick={onClose}>
      <div className="flex max-h-[85vh] w-full max-w-[640px] flex-col overflow-hidden rounded-2xl bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="border-b border-slate-100 p-5">
          <div className="mb-1 text-[16px] font-semibold text-slate-800">Chọn tài liệu từ Kho — Khối {khoi} · {mon}</div>
          <p className="mb-3 text-[12px] text-slate-400">Thường là MT, nhưng không giới hạn loại tài liệu.</p>
          <input className={inputCls} value={q} onChange={(e) => setQ(e.target.value)} placeholder="Tìm theo tên…" autoFocus />
          <label className="mt-2 flex items-center gap-1.5 text-[12px] text-slate-500"><input type="checkbox" checked={taiKhoi} onChange={(e) => setTaiKhoi(e.target.checked)} />Chỉ khối {khoi} (bỏ tick để xem mọi khối)</label>
        </div>
        <div className="min-h-0 flex-1 overflow-y-auto p-3">
          {loading ? <p className="p-4 text-sm text-slate-400">Đang tải…</p> : filtered.length === 0 ? (
            <p className="p-4 text-sm text-slate-400">Không tìm thấy tài liệu khớp.</p>
          ) : (
            <div className="space-y-1.5">
              {filtered.map((t) => (
                <button key={t.id} onClick={() => chon(t)} className="flex w-full items-center gap-2 rounded-lg border border-slate-100 p-2.5 text-left hover:border-indigo-300 hover:bg-indigo-50/40">
                  ...
                  <span className="shrink-0 text-[11px] text-slate-400">K{t.khoi}</span>
                </button>
              ))}
            </div>
          )}
