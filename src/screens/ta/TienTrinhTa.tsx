...
const CHI_SO: { key: Key; label: string; icon: string }[] = [
  { key: 'buoi', label: 'Trợ giảng buổi', icon: P('tro_giang') }, { key: 'btvn', label: 'Chấm BTVN', icon: P('btvn') }, { key: 'et', label: 'Chấm ET', icon: P('et') },
]
...
const ST_CLS: Record<St, { pill: string; text: string; icon: string; ring: string }> = {
  dat: { pill: 'bg-[#E4F8EC]', text: 'text-[#1E8A52]', icon: P('star'), ring: BK.success },
  thieu: { pill: 'bg-[#FFF1D6]', text: 'text-[#C27A00]', icon: P('warn_cam'), ring: BK.warning },
  nguy: { pill: 'bg-[#FFE3EA]', text: 'text-[#C0355A]', icon: P('warn_do'), ring: BK.danger },
}

// tile KPI trắng như design: icon + nhãn · vòng % + thực/chuẩn · pill trạng thái
function Tile({ icon, label, th, ch, unit }: { icon: string; label: string; th: number; ch: number; unit?: string }) {
  const lech = th - ch
  ...
  return (
    <div className="flex flex-col items-center rounded-2xl bg-white/95 px-1 py-1.5 text-center">
      <p className="flex items-center gap-1 text-[9.5px] font-bold leading-tight text-[#16224D]"><img src={icon} alt="" className="h-3.5 w-3.5 object-contain" draggable={false} />{label}</p>
      ...
      <span className={`mt-1 flex items-center gap-1 rounded-full px-1.5 py-0.5 text-[9px] font-bold ${c.pill} ${c.text}`}>
        <img src={c.icon} alt="" className="h-3 w-3 object-contain" draggable={false} />{st === 'dat' ? 'Đạt chuẩn' : unit ? `Thiếu ${fmt(-lech)}${unit}` : st === 'thieu' ? 'Thiếu 1 lần' : `Thiếu ${-lech} lần`}
      </span>
    </div>
  )
}
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-1">
      ...
      <div className="relative overflow-hidden rounded-[20px] px-2.5 py-2" style={{ background: '#D6EDFC' }}>
        ...
        <div className="mt-1 flex items-center gap-2">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 text-[9.5px] leading-snug text-[#63709A]">Chuẩn hợp đồng: {d.dinh_muc.buoi_x_tuan}×buổi/tuần · BTVN {d.dinh_muc.btvn} · ET {d.dinh_muc.et} · bổ trợ {d.dinh_muc.botro_gio}h. "Thực" đếm tới hôm nay.</p>
          </div>
          <img src={P('chart')} alt="" className="h-12 w-11 shrink-0 object-contain" draggable={false} />
          <img src={P('mascot_note')} alt="" className="h-[70px] w-[46px] shrink-0 rounded-xl object-cover" draggable={false} />
        </div>
        <p className="mt-1 flex items-center gap-1 rounded-xl bg-white/70 px-2 py-1 text-[9.5px] text-[#63709A]"><img src={P('info')} alt="" className="h-3.5 w-3.5" draggable={false} />Bảng này để bạn theo dõi thừa/thiếu so với hợp đồng — không tính lương tại đây.</p>
      </div>
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-1 overflow-y-auto">
              ...
              <div key={l.lop_id} className="rounded-[20px] p-2" style={{ background: tone.bg }}>
                <div className="mb-1.5 flex items-center gap-2">
                  ...
                  <div className={`flex items-center gap-1.5 rounded-full px-2 py-1 ${sc.pill}`}>
                    <img src={sc.icon} alt="" className="h-5 w-5 object-contain" draggable={false} />
        ...
        <div className="rounded-[20px] p-2" style={{ background: '#E6DDFF' }}>
          <div className="mb-1.5 flex items-center gap-2">
            <img src={P('bo_tro')} alt="" className="h-9 w-9 shrink-0 rounded-xl object-cover" draggable={false} />
            <div className="min-w-0 flex-1 leading-tight">
          ...
          <div className="grid grid-cols-3 gap-1"><Tile icon={P('bo_tro')} label="Giờ bổ trợ" th={d.botro.thuc_gio} ch={d.botro.chuan_gio} unit="h" /></div>
        </div>
      </div>
    </div>
  )
