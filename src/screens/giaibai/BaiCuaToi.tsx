...
import { listCuaToi, luuNhap, nopBai, traBai, conLai, fmtTs, fmtGiay, TRANG_THAI_LABEL, laHinh, type ChuoiHinh, type DongNhan, type NoiDungGiai } from '../../lib/giaibai'
...
export default function BaiCuaToi({ me, onChanged }: { me: string; onChanged: () => void }) {
  ...
  const chuoi = useChuoi([...dang, ...choDuyet])   // Hình: chuỗi (đích + tiền đề) — đọc + soạn theo từng ý
  const chuoiCua = (r: DongNhan) => chuoi.get(chuoiKey(r.nhanh, r.key))
  ...
  const onLuu = (r: DongNhan) => (a: NoiDungGiai) => luuNhap(r.nhanh, r.id, me, a).then(() => setRows((rs) => rs.map((x) => x.id === r.id ? { ...x, loi_giai_nhap: a.loiGiai, anh_nhap: a.anh, dap_an_nhap: a.dapAn, y_nhap: a.yNhap ?? null } : x)))
  ...
  const the = (r: DongNhan, children?: React.ReactNode) => (
    <The key={r.id} r={r} chuoi={chuoiCua(r)} busy={busyId === r.id} open={openId === r.id} onTra={() => onTra(r)} onToggle={() => setOpenId(openId === r.id ? null : r.id)}>{children}</The>
  )
      ...
      {loading ? <p className="text-sm text-slate-400">Đang tải…</p> : err ? <p className="text-sm text-rose-600">Lỗi: {err}</p> : (
        <>
          <Section title="Đang giải" count={dang.length} hint="tối đa 3 bài · hạn 48h kể từ lúc nhận / lúc bị từ chối" empty="Chưa nhận bài nào — sang tab Kho bài để nhận.">
            {dang.map((r) => the(r, openId === r.id && (laHinh(r.nhanh) && !chuoiCua(r)
              ? <p className="mt-3 text-[13px] text-slate-400">Đang tải chuỗi bài…</p>
              : <GiaiEditor key={r.id} initial={{ loiGiai: r.loi_giai_nhap, anh: r.anh_nhap, dapAn: r.dap_an_nhap, yNhap: r.y_nhap ?? undefined }}
                  hoiDapAn={!laHinh(r.nhanh) && !r.dap_an} tieuDe={`Lời giải · ${r.ma}`} chuoi={chuoiCua(r)}
          ...
          <Section title="Chờ duyệt" count={choDuyet.length} hint="team học thuật đang xem" empty="Không có bài chờ duyệt.">
            {choDuyet.map((r) => the(r, <LoiGiaiDaNop r={r} chuoi={chuoiCua(r)} />))}
          </Section>
...
function The({ r, chuoi, busy, open, onTra, onToggle, children }: { r: DongNhan; chuoi?: ChuoiHinh; busy: boolean; open: boolean; onTra: () => void; onToggle: () => void; children?: React.ReactNode }) {
  const han = conLai(r.han_at)
  const dangMo = r.xu_ly_at === null && (r.trang_thai === 'dang_giai' || r.trang_thai === 'can_sua')
  const coNhap = !!(r.loi_giai_nhap || r.anh_nhap || r.y_nhap?.length)
  return (
    <li className={`rounded-xl border bg-white p-4 shadow-sm ${r.trang_thai === 'can_sua' ? 'border-rose-200' : 'border-slate-200'}`}>
      <BaiHead b={r} right={<>
        ...
        {dangMo && !r.qua_han && <>
          ...
          <button onClick={onToggle} disabled={busy}
            ...
            {open ? 'Đóng' : coNhap ? '✍️ Sửa tiếp' : '✍️ Soạn lời giải'}
          </button>
        </>}
      </>} />
      ...
      {/* Hình: khi ĐANG SOẠN, chuỗi đã nằm trong ChuoiSoan (mỗi ý 1 ô) — không hiện lặp; lúc khác hiện cả chuỗi để đọc */}
      {chuoi ? (!(open && dangMo) && <ChuoiDoc chuoi={chuoi} compact={!dangMo} />) : <BaiBody b={r} compact={!dangMo} />}
      {children}
    </li>
  )
}
function LoiGiaiDaNop({ r, chuoi }: { r: DongNhan; chuoi?: ChuoiHinh }) {
  return (
    <div className="mt-3 rounded-lg border border-slate-200 bg-slate-50 p-3">
      <div className="mb-1 flex items-center gap-2 text-[11px] font-semibold uppercase tracking-wide text-slate-500">
        Lời giải đã nộp <span className="font-normal normal-case text-slate-400">· nộp {fmtTs(r.nop_at)} · giải trong {fmtGiay(r.giay_giai)} · {r.so_ky_tu} ký tự · {r.so_cong_thuc} công thức{r.y_nhap?.length ? ` · ${r.y_nhap.length} ý` : ''}</span>
      </div>
      {chuoi && r.y_nhap?.length ? <YNhapDoc chuoi={chuoi} yNhap={r.y_nhap} /> : (
