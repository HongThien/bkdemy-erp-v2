  ...
  async function gui() {
    ...
    try {
      ...
      await createBaoLoi({
        mo_ta: moTa.trim(),
        route: `app_gv:${route}`,
        context: { app: 'gv', email: user?.email, url: location.href, viewport: `${innerWidth}x${innerHeight}`, ua: navigator.userAgent, errors: recentErrors() },
      })
      setDone(true)
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-900/40 p-3 sm:items-center" onClick={() => setOpen(false)}>
          <div className="w-full max-w-[440px] rounded-2xl bg-white p-4 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            {done ? <p className="py-6 text-center text-[14px] font-bold text-emerald-600">✓ Đã gửi — cảm ơn bạn!</p> : (
              <>
                <p className="mb-1 text-[14px] font-bold text-slate-900">🐞 Góp ý / báo lỗi app Giáo viên</p>
                ...
                <textarea value={moTa} onChange={(e) => setMoTa(e.target.value)} autoFocus
                  placeholder="Vd: màn đánh giá lớp 8S0 lưu mức bị chậm…"
                ...
                <div className="flex justify-end gap-2">
                  ...
                  <button onClick={gui} disabled={busy} className="min-h-[40px] rounded-lg bg-green-600 px-4 text-[13px] font-semibold text-white disabled:opacity-40">{busy ? 'Đang gửi…' : 'Gửi'}</button>
                </div>
              </>
            )}
          </div>
